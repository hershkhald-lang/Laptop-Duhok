...
(function () {
  ...
  function pick(row, base) {
    ...
    var ar = row[base + "_ar"];
    var ckb = row[base + "_ckb"];
    var bad = row[base + "_bad"];
    if (lang === "ckb" && ckb && String(ckb).trim()) return String(ckb);
    if (lang === "bad" && bad && String(bad).trim()) return String(bad);
    return String(ar || ckb || bad || "");
  }
  ...
  function specsHtml(row) {
    ...
    var raw =
      lang === "ckb"
        ? (row.specs_ckb || "").trim()
        : lang === "bad"
        ? (row.specs_bad || row.specs_ckb || "").trim()
        : (row.specs_ar || "").trim();
    if (!raw) return "";
