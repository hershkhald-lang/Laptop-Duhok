...
(function () {
  ...
  function currentLang() {
    return document.documentElement.lang === "ckb" ? "ckb" : "ar";
  }

  function pickTitle(row) {
    if (currentLang() === "ckb" && (row.title_ckb || "").trim()) return row.title_ckb.trim();
    return (row.title_ar || row.title_ckb || "").trim() || "—";
  }

  function pickDesc(row) {
    if (currentLang() === "ckb" && (row.description_ckb || "").trim()) return row.description_ckb.trim();
    return (row.description_ar || row.description_ckb || "").trim() || "";
  }
  ...
  function renderEpisodes(filterCat) {
    ...
    if (!list.length) {
      ...
      if (!sourceRows.length) {
        emptyMsg = usedDatabase
          ? 'لا توجد حلقات منشورة بعد. أضف فيديوهات من <strong>لوحة التحكم ← شروحات الكاشير</strong>.'
          : 'لا توجد بيانات بعد. نفّذ <code>supabase/cashier-videos.sql</code> ثم أضف فيديوهات من لوحة التحكم، أو تحقق من الاتصال.';
      } else {
    ...
    list.forEach(function (ep) {
      var yid = ep.youtube_id;
      var title = pickTitle(ep);
      var desc = pickDesc(ep);
      var idx = ep._episodeNum || 1;
  ...
  function start() {
    ...
    if (window.location.protocol === "file:") {
      var fileWarn = document.getElementById("ct-protocol-warning");
