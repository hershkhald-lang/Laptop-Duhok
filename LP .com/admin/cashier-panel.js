...
(function () {
  ...
  /** معرّف الفيديو: 11 حرفاً؛ يُستخرج من رابط قديم في قاعدة البيانات فقط */
  function extractYoutubeId(raw) {
    if (window.LD_extractYoutubeVideoId) {
      var id = window.LD_extractYoutubeVideoId(raw);
      if (id) return id;
    }
    var s = (raw || "").trim();
    if (!s) return "";
    if (/^[a-zA-Z0-9_-]{11}$/.test(s)) return s;
    var m = s.match(/(?:[?&]v=|\/embed\/|youtu\.be\/|shorts\/|live\/)([a-zA-Z0-9_-]{11})/);
    return m ? m[1] : "";
  }
  ...
  function fillForm(row) {
    editingId = row.id;
    $("cv-id").value = row.id;
    $("cv-youtube").value = extractYoutubeId(row.youtube_id) || "";
  ...
  async function saveCashier() {
    ...
    var rawIn = ($("cv-youtube").value || "").trim();
    var yid = extractYoutubeId(rawIn);
    if (!yid || !/^[a-zA-Z0-9_-]{11}$/.test(yid)) {
      setCvMsg("أدخل معرّف الفيديو المكوّن من 11 حرفاً بالضبط (مثل mdpefNOFqoM).", "is-error");
      return;
    }
    ...
    client
      ...
      .then(function (res) {
        ...
        rows.forEach(function (row) {
          var clean = extractYoutubeId(row.youtube_id);
          var img = clean
            ? '<img src="' + esc(thumbUrl(clean)) + '" class="cv-list-thumb" alt="" />'
            : "—";
          html +=
            "<tr><td>" +
            img +
